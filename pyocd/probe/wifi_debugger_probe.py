...
class WiFiDebuggerProbe(DebugProbe):
    ...
    def get_all_connected_probes(cls, unique_id=None, is_explicit=False):
        ...
        list = []
        for pt in pts:
            if 'USB JTAG/serial debug unit' in pt[1]: #check 'USB' string in device description
    ...
    def __init__(self, ttyDev):
        """@brief Constructor."""
        super(WiFiDebuggerProbe, self).__init__()
...
class WiFiDebuggerProbePlugin(Plugin):
    """@brief Plugin class for WiFiDebuggerProbePlugin."""

    def load(self):
        return WiFiDebuggerProbe
