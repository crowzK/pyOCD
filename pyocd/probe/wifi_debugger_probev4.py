...
class WiFiDebuggerProbeV4(DebugProbe):
    ...
    def get_all_connected_probes(cls, unique_id=None, is_explicit=False):
        ...
        list = []
        for pt in pts:
            if 'wifi debugger' in pt[1]: #check 'USB' string in device description
                list.append(cls(pt[0]))
        return [list[0]]
    ...
    def __init__(self, ttyDev):
        """@brief Constructor."""
        super(WiFiDebuggerProbeV4, self).__init__()

        self._uid = ttyDev
        self.serial = serial.Serial()
        self.serial.baudrate = 230400
        self.serial.port = ttyDev
        self._is_open = False
        self._request_id = 0
        self._lock_count = 0
...
class WiFiDebuggerProbeV4Plugin(Plugin):
    """@brief Plugin class for WiFiDebuggerProbeV4Plugin."""

    def load(self):
        return WiFiDebuggerProbeV4

    @property
    def name(self):
        return "wifidebuggerV3"
